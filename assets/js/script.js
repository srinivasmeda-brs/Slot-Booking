...
const event_available_slots_api = "https://api.calendly.com/event_type_available_times";
...
function openCalendlyPopup(url) {
  ...
  Calendly.on('popupClose', function() {
...
function getAvailableTimes(date) {
  ...
  let end_date = moment(date, 'YYYY-MM-DD').endOf('day').format('YYYY-MM-DDTHH:mm:ss');
  ...
  const event_avialable_times_url = `${event_available_slots_api}?${new URLSearchParams(event_avialable_times_data)}`;
  ...
  .then(data => {
    ...
    if (data.collection?.length) {
      let slots_list = slots_data.map(e => {
        return (`
          <div class="row justify-content-md-center">
            <div class="col col-6 time-slot text-center m-2">
              <a href="#" onclick="openCalendlyPopup('${e.scheduling_url}'); return false;">${moment(e.start_time).format('hh:mm A')}</a>
            </div>
          </div>
        `);
      }).join('');
      selected_date.innerHTML = `<b>${moment(date, 'YYYY-MM-DD').format('ddd, MMM DD')}<b>`;
